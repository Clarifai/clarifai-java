 ...
 */
public interface ClarifaiJsonRequest<RESULT> {
  /**
   ...
   * {@link ClarifaiJsonRequest#executeAsync(Callback)} is called on a {@link ClarifaiJsonRequest}
   *
  ...
  abstract class Adapter<T> implements ClarifaiJsonRequest<T> {
  ...
  abstract class Builder<T> extends Adapter<T> {
    ...
    @NotNull protected ClarifaiJsonRequest<T> build() {
